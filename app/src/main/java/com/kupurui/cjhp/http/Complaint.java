...
public class Complaint {
    ...
    public void index(String u_id, String title, String content, String tel, List<File> image, HttpListener listener , int what){
        ...
        for (int i = 0; i < image.size(); i++) {
            params.addParam("image["+i+"]",image.get(i));
        }
    ...
    //已处理详情
    public void detail(String c_id,HttpListener listener ,int what){
        RequestParams params = new RequestParams();
        params.addParam("c_id",c_id);
        new HttpUtils().doCall(AppConfig.BASE_URL+module+"/detail",params,listener,what);
    }
    //评论
    public void comment(String c_id,String star,String comment,HttpListener listener,int what){
        RequestParams params = new RequestParams();
        params.addParam("c_id",c_id);
